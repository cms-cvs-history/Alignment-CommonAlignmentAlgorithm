...
BzeroReferenceTrajectory::BzeroReferenceTrajectory(const TrajectoryStateOnSurface &refTsos,
						   ...
						   MaterialEffects materialEffects, double mass, double momentumEstimate) :
  ReferenceTrajectory( refTsos.localParameters().mixedFormatVector().num_row(), recHits.size() ),
  theMomentumEstimate( momentumEstimate )
{
  // no check against magField == 0

  // No estimate for momentum available -> set to default value.
  theParameters = refTsos.localParameters().mixedFormatVector();
  theParameters[0] = 1./theMomentumEstimate;

  LocalTrajectoryParameters locParamWithFixedMomentum( theParameters, refTsos.localParameters().pzSign(), true );
